...
def load_split_embed_docs(loader,splitter,summary,embedder):
        ...
        table_name="test1"
        distance_strategy=st.session_state.distance_metric
...
def suggestTableName():
    ...
    table_name = st.text_input(label="Provide a Table Name for Vector Store",value="TBL_"+get_selected_distance())
    if len(table_name)<1:
        st.warning("Provide a Table Name")
        return None
    else:
        return table_name
...
with tab1:
    ...
    with c1:#st.expander("Load Documents",expanded=True):  
        localdir="./Data/docs"
        loader_params = {}        
        ...
        if selected_loader_type == "File":
            ...
            if (selected_table_name==""):
                ...
                selected_column_name = st.selectbox(
                    "Existing Columns",
                    (db_ops.get_columns_list(selected_table_name)),
                    index=None,
        ...
        if st.button("Load & Create Vectors","Loader"):
            ...
            summary_params  = {"provider": "database", "model": st.session_state.embedding_model} 
    ...
    with col2:
        with st.expander("Index Parameters"): 
            if selected_index_type == "HNSW":
                distance = st.session_state.distance_metric # for hnsw EUCLIDEAN, L2_SQUARED (aka EUCLIDEAN_SQUARED), COSINE, DOT, MANHATTAN, HAMMING
                #if hnsw index
